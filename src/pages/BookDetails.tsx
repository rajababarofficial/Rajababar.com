...
export default function BookDetails() {
  ...
  if (!book) return <div className="text-center py-20 bg-brand-bg min-h-screen font-sindhi">ڪتاب نه مليو.</div>;

  return (
    <div dir={isSindhi ? "rtl" : "ltr"} className="min-h-screen pt-24 pb-12 bg-brand-bg px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        
        {/* Navigation - Fixed for better mobile tap area */}
        <Link to="/library" className="inline-flex items-center gap-2 text-brand-secondary hover:text-brand-accent mb-8 transition-all group py-2">
          {isSindhi ? <ArrowRight className="w-5 h-5" /> : <ArrowLeft className="w-5 h-5" />}
          <span className={cn("font-bold text-sm", isSindhi && "font-sindhi text-xl")}>{isSindhi ? "واپس لائبريري" : "Back to Library"}</span>
        </Link>

        <div className="flex flex-col lg:flex-row gap-8 lg:gap-12">
          
          {/* Left: Book Cover - Mobile Optimized Size */}
          <div className="w-full lg:w-[380px] shrink-0">
            ...
            >
              {book.thumbnail ? (
                ...
                <div className="w-full h-full flex items-center justify-center opacity-10 bg-brand-bg"><BookIcon className="w-24 h-24" /></div>
              )}
            </motion.div>
          </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            {/* Title & Author - Dual Language Compact */}
            <div className="space-y-8">
              {/* Sindhi Info */}
              <div className="space-y-2">
                <h1 className="font-sindhi text-4xl sm:text-5xl lg:text-6xl text-brand-primary leading-[1.3]">
                  {book.title_sd}
                </h1>
              ...
              {/* English Info - Always LTR for alignment */}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 p-6 bg-brand-surface/40 border border-brand-border rounded-2xl backdrop-blur-sm">
               <div className="space-y-1">
                  ...
                  {/* Publisher is often in English, so we force LTR for it if needed */}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  ...
                  className="flex items-center justify-center gap-2 h-14 bg-brand-surface border border-brand-border rounded-xl hover:border-brand-accent transition-all relative overflow-hidden text-xs font-bold"
                >
